...
<!-- 
	...
	같은 페이지내에서 서로 값을 공유 할 수 있으며, 주요기능은 다른 기본 객체들을 구할 때 사용하거나
	페이지 흐름을 제어할 때 사용한다.
	...
	** 속성을 이용해서 데이터 저장 및 호출(생명주기가 해당 페이지를 벗어나면 사라진다.)
	저장 방법 : pageContext.setAttribute("이름", 객체);
	호출 방법 : pageContext.getAttribute("이름");
	생명주기  : 해당 페이지를 벗어나면 저장정보도 사라진다.
	
	** 속성을 이용해서 데이터를 저장 및 호출 하는 내장 객체들
	...
	생명주기 : pageContext < request < session < application
			   ...
			   session : 해당 브라우저가 종료되면 저장정보도 사라진다.
			   			 (예외적으로 시간을 저장할 수 있다.)
			   			 로그인 정보 처리는 무조건 session 정보에 저장해야 된다.
			   application : 어플리케이션이 종료되기 전까지 정보는 사라지지 않는다.
			   				 어플리케이션 외부 환경정보(context)를 나타내는 내부 객체이다.
